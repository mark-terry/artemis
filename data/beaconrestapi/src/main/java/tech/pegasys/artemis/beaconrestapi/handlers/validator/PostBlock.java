...
public class PostBlock implements Handler {
  ...
  public void handle(final Context ctx) throws Exception {
    try {
      ...
      final ValidatorBlockResult blockImportResult =
          validatorDataProvider.submitSignedBlock(signedBeaconBlock);

      ctx.status(blockImportResult.getResponseCode());
      if (blockImportResult.getFailureReason().isPresent()) {
        ctx.result(
            jsonProvider.objectToJSON(blockImportResult.getFailureReason().get().getMessage()));
      } else {
        ctx.result(jsonProvider.objectToJSON(blockImportResult.getHash_tree_root()));
      }
